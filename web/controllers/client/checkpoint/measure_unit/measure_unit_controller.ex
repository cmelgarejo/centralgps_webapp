defmodule CentralGPSWebApp.Client.Checkpoint.MeasureUnitController do
  use CentralGPSWebApp.Web, :controller
  import CentralGPS.RestClient
  import CentralGPS.Repo.Utilities

  # POST    /checkpoint/measure_units/create
  # GET     /checkpoint/measure_units/:measure_unit_id
  # PUT     /checkpoint/measure_units/:measure_unit_id
  # DELETE  /checkpoint/measure_units/:measure_unit_id
  # GET     /checkpoint/measure_units
  # GET     /checkpoint/measure_units/json
  ...
  #private functions
  defp api_method(action \\ "") when is_bitstring(action), do: "/checkpoint/measure_unit/" <> action
  ...
  defp save_record(s, p) do
    ...
    if (String.to_atom(p.__form__) ==  :edit) do
      data = %{measure_unit_id: p.id, configuration_id: s.configuration_id, name: p.name,  notes: p.notes }
      {_, res} = api_put_json api_method(data.measure_unit_id), s.auth_token, s.account_type, data
    else
      data = %{ configuration_id: s.configuration_id, name: p.name, notes: p.notes }
